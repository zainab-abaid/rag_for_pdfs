...
Env:
  ...
  RERANKER_MODE=none|entity|rrf|mmr|custom  # REQUIRED: reranker mode
...
from src.postfiltering.product_postfilter import apply_postfilter, get_postfilter_mode
from src.reranking.ner_extras import augment_entities
...
def stitch_context(
    ...
    ranked_all = sorted(
        best_by_section.items(),
        key=lambda kv: getattr(kv[1], "score", 0.0),
        reverse=True,
    )
    ...
    try:
        order = rerank_fn(query, candidates, [b.get("orig_score", 0.0) for b in candidates])
        if not isinstance(order, list) or len(order) != len(candidates):
            order = list(range(len(candidates)))
    except Exception:
        order = list(range(len(candidates)))

    all_post_pretrim = [candidates[i] for i in order]

    # 7) trim
    trimmed_post = all_post_pretrim[:final_k]
    return trimmed_post, all_post_pretrim, all_pre_pretrim
    ...
    with open(out_csv, "w", encoding="utf-8", newline="") as f_out:
        ...
        for i, row in enumerate(rows, 1):
           ...
           # print(f"augmented ents: {ents}")
            ...
            blocks_trimmed_post, blocks_all_post, blocks_all_pre = stitch_context(
